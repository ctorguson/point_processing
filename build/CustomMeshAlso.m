...
function [vertices, faces] = read_off_file(filename)
    ...
    if ~strcmp(header, 'OFF')
        fclose(fileID); % Ensure the file is closed before throwing an error
    ...
    % Preallocate the cell array to hold face data
    faces = cell(numFaces, 1);
    
    for i = 1:numFaces
        % Read the number of vertices for this face (n) and then n vertex indices
        faceInfo = fscanf(fileID, '%d', 1);
        faceVertices = fscanf(fileID, '%d', faceInfo);
        
        % Adjust for MATLAB's 1-based indexing
        faces{i} = faceVertices' + 1;
    end
    
    % Convert faces from a cell array to a numeric array if desired
    % Note: This step is optional and depends on how you intend to use the faces data.
    % If keeping faces as a cell array works for your application, you can skip this.
