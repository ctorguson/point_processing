...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    std::ofstream out("/Users/ciaratorguson/point_processing/build/output_mesh.off");
    ...
    // Verify file writing success (After finishing writing to the file)
    if (!out) {
    mexErrMsgIdAndTxt("PointProcessing:FileOpenError", "Failed to open output_mesh.off for writing.");
    return;
}

    // Properly close the file (Immediately after the verification)
    out.close();
